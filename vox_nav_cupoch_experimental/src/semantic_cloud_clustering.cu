...
struct denoise_cloud_functor
{
        ...
        __host__ __device__ void operator()(Eigen::Vector3f &i)
        {
...
SemanticCloudClustering::SemanticCloudClustering()
    : Node("dynamic_points_node"), recieved_first_(false)
{
        ...
        cloud_odom_data_approx_time_syncher_->registerCallback(
            std::bind(
                &SemanticCloudClustering::cloudOdomCallback, this,
                std::placeholders::_1,
                std::placeholders::_2));
...
void SemanticCloudClustering::cloudOdomCallback(
...
void SemanticCloudClustering::determineObjectMovements(
...
void SemanticCloudClustering::clusterSet2CloudVector(
...
void SemanticCloudClustering::clusterIndices2ClusterSet(
...
std::shared_ptr<cupoch::geometry::PointCloud> SemanticCloudClustering::denoiseCupochCloud(
    std::shared_ptr<cupoch::geometry::PointCloud> &a,
    ...
    int max_nn)
{
        ...
        int counter = 0;
        denoise_cloud_functor f(&radius, &max_nn, &a_colors, &kdtree, &counter, &points, &colors);
...
Eigen::Matrix4f SemanticCloudClustering::getTransfromfromConsecutiveOdoms(
...
int main(int argc, char const *argv[])
{
        ...
        auto node = std::make_shared<SemanticCloudClustering>();
        rclcpp::spin(node);
        rclcpp::shutdown();
        return 0;
}
