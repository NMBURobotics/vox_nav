...
for s in range(0, goal[0], step):
    waypoints.append([s, s])
waypoints.append(goal)

costmap = cv.imread("aruco.png")
...
distance_transfrom_inverted = cv.distanceTransform(
    binary_thresh_inverted, distanceType=cv.DIST_L2, maskSize=kernel_size)

distance_transfrom = cv.distanceTransform(
    binary_thresh, distanceType=cv.DIST_L2, maskSize=kernel_size)

distance_transfrom_inverted = cv.normalize(
    distance_transfrom_inverted, distance_transfrom_inverted, 0.0, 1.0, cv.NORM_MINMAX)

distance_transfrom = cv.normalize(
    distance_transfrom, distance_transfrom, 0, 1.0, cv.NORM_MINMAX)

np.set_printoptions(threshold=sys.maxsize)

gradx_dti = cv.Sobel(distance_transfrom_inverted,
                     cv.CV_32F, 1, 0, ksize=kernel_size)
grady_dti = cv.Sobel(distance_transfrom_inverted,
                     cv.CV_32F, 0, 1, ksize=kernel_size)
gradx_dt = cv.Sobel(distance_transfrom, cv.CV_32F, 1, 0, ksize=kernel_size)
grady_dt = cv.Sobel(distance_transfrom, cv.CV_32F, 0, 1, ksize=kernel_size)
...
for i in range(0, gradx_dt.shape[0], 1):
    for j in range(0, grady_dt.shape[0], 1):
