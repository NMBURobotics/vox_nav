...
PlannerBenchMarking::PlannerBenchMarking()
  ...
  this->declare_parameter("selected_planners",
  ...
  this->get_parameter("min_euclidean_dist_start_to_goal",
  ...
  this->get_parameter("sample_bencmark_plans_topic",
                      sample_bencmark_plans_topic_);

  typedef std::shared_ptr<fcl::CollisionGeometry> CollisionGeometryPtr_t;
  CollisionGeometryPtr_t robot_body_box(new fcl::Box(robot_body_dimensions_.x,
...
PlannerBenchMarking::doBenchMarking() {
  ...
  for (int i = 0; i < epochs_; i++) {
    ...
    while (!found_valid_random_start_goal) {
      ...
      double distance =
          std::sqrt(std::pow(random_goal->getX() - random_start->getX(), 2) +
      ...
      si->setStateValidityCheckingResolution(1.0 /
      ...
      RCLCPP_INFO(this->get_logger(), "Checking whether a solution exists for "
    ...
    benchmark.saveResultsToFile((results_output_dir_ + results_file_regex_ +
...
PlannerBenchMarking::makeAPlan(const ompl::base::PlannerPtr &planner,
...
void PlannerBenchMarking::setupMap() {
  ...
  while (!is_map_ready_ && rclcpp::ok()) {
    ...
    while (!get_maps_and_surfels_client_->wait_for_service(
      ...
      RCLCPP_INFO(this->get_logger(), "get_maps_and_surfels service not "
...
int main(int argc, char **argv) {
  ...
  while (rclcpp::ok() && !node->is_map_ready_) {
    ...
    RCLCPP_INFO(node->get_logger(), "Waiting for octomap to be ready In order "
