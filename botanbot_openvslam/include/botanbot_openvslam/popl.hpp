...
public:
  ...
  Value(
    const std::string & short_name, const std::string & long_name,
    const std::string & description, const T & default_val, T * assign_to = nullptr);
...
public:
  Implicit(
    const std::string & short_name, const std::string & long_name,
    const std::string & description, const T & implicit_val, T * assign_to = nullptr);
...
public:
  ...
  invalid_option(
    const Option * option, invalid_option::Error error, OptionName what_name,
    std::string value, const std::string & text)
  : std::invalid_argument(text.c_str()), option_(option), error_(error), what_name_(what_name),
...
public:
  ...
  explicit OptionPrinter(const OptionParser * option_parser)
  : option_parser_(option_parser)
  {
  }
...
inline Value<T>::Value(
  const std::string & short_name, const std::string & long_name,
  const std::string & description, const T & default_val, T * assign_to)
...
inline void Value<std::string>::parse(OptionName what_name, const char * value)
{
  if (strlen(value) == 0) {
    throw invalid_option(
            this, invalid_option::Error::missing_argument, what_name, value, "missing argument for " + name(
              what_name,
              true));
  }
...
inline void Value<T>::parse(OptionName what_name, const char * value)
{
  ...
  if (is.fail()) {
    throw invalid_option(
            this, invalid_option::Error::invalid_argument, what_name, value, "invalid argument for " + name(
              what_name,
              true) + ": '" + strValue + "'");
  }
  ...
  if (valuesRead > 1) {
    throw invalid_option(
            this, invalid_option::Error::too_many_arguments, what_name, value, "too many arguments for " + name(
              what_name,
              true) + ": '" + strValue + "'");
  }
  ...
  if (strValue.empty()) {
    throw invalid_option(
            this, invalid_option::Error::missing_argument, what_name, "", "missing argument for " + name(
              what_name,
              true));
  }
...
template<class T>
inline Implicit<T>::Implicit(
  const std::string & short_name, const std::string & long_name,
  const std::string & description, const T & implicit_val, T * assign_to)
...
inline Switch::Switch(
  const std::string & short_name, const std::string & long_name,
  const std::string & description, bool * assign_to)
...
inline OptionParser::OptionParser(std::string description)
: description_(std::move(description))
{
}
...
inline void OptionParser::parse(int argc, const char * const argv[])
{
  ...
  for (auto & opt : options_) {
    if ((opt->attribute() == Attribute::required) && !opt->is_set()) {
      std::string option =
        opt->long_name().empty() ? std::string(1, opt->short_name()) : opt->long_name();
      throw invalid_option(
              opt.get(), invalid_option::Error::missing_option,
...
inline ConsoleOptionPrinter::ConsoleOptionPrinter(const OptionParser * option_parser)
: OptionPrinter(option_parser)
{
}
...
inline GroffOptionPrinter::GroffOptionPrinter(const OptionParser * option_parser)
: OptionPrinter(option_parser)
{
}
...
inline BashCompletionOptionPrinter::BashCompletionOptionPrinter(
  const OptionParser * option_parser,
