    ...
    const sensor_msgs::msg::PointCloud2::ConstSharedPtr cloud)
{
  if (map_configured_)
  {
        ...
        std::abs(diff(2, 3)) > 0.1)
    {
      ...
      res.transformation_(0, 3) = clamp<double>(diff(0, 3), -0.1, 0.1);
      res.transformation_(1, 3) = clamp<double>(diff(1, 3), -0.1, 0.1);
      res.transformation_(2, 3) = clamp<double>(diff(2, 3), -0.1, 0.1);
    }
