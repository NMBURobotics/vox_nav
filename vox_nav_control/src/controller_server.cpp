  ...
  , plan_refiner_type_("vox_nav_control::TraversabilityBasedPlanRefiner")
{
  ...
  try
  {
    controller_ = pc_loader_.createSharedInstance(controller_type_);
    controller_->initialize(this, controller_id_);
    RCLCPP_INFO(get_logger(), "Created and initialized controller plugin %s of type %s", controller_id_.c_str(),
  ...
  RCLCPP_INFO(get_logger(), "Selected controller id: %s", controller_id_.c_str());
  ...
  try
  {
    plan_refiner_ = pr_loader_.createSharedInstance(plan_refiner_type_);
    plan_refiner_->initialize(this, plan_refiner_id_);
    RCLCPP_INFO(get_logger(), "Created and initialized plan refiner plugin %s of type %s", plan_refiner_id_.c_str(),
                plan_refiner_type_.c_str());
  }
  catch (const pluginlib::PluginlibException& ex)
  {
    RCLCPP_FATAL(get_logger(), "Failed to create plan refiner. Exception: %s", ex.what());
  }
  ...
  this->action_server_ = rclcpp_action::create_server<FollowPath>(
      ...
      this->get_node_waitables_interface(), "follow_path",
  ...
  cmd_vel_publisher_ = this->create_publisher<geometry_msgs::msg::Twist>("vox_nav/cmd_vel", 10);

  // Initialize pubs & subs
  plan_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("vox_nav/planning/plan", 1);

  nav_msgs_path_pub_ =
      this->create_publisher<nav_msgs::msg::Path>("vox_nav/planning/nav_msgs_path", rclcpp::SystemDefaultsQoS());
...
void ControllerServer::executePathRefinerThread()
{
  while (rclcpp::ok())
  {
    ...
    if (plan_refiner_->refinePlan(curr_robot_pose, *global_path_))
    {
      ...
      scale.y = 0.2;
      scale.z = 0.2;
      vox_nav_utilities::publishPlan(global_path_->poses, global_path_->poses.front(), global_path_->poses.back(),
                                     scale, plan_publisher_, nav_msgs_path_pub_);
    }
...
rclcpp_action::GoalResponse ControllerServer::handle_goal(const rclcpp_action::GoalUUID& uuid,
                                                          std::shared_ptr<const FollowPath::Goal> goal)
{
  RCLCPP_INFO(this->get_logger(), "Received goal request in order to follow a path");
  (void)uuid;
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse ControllerServer::handle_cancel(const std::shared_ptr<GoalHandleFollowPath> goal_handle)
{
...
void ControllerServer::handle_accepted(const std::shared_ptr<GoalHandleFollowPath> goal_handle)
{
  ...
  std::thread{ std::bind(&ControllerServer::followPath, this, std::placeholders::_1), goal_handle }.detach();
}
...
void ControllerServer::followPath(const std::shared_ptr<GoalHandleFollowPath> goal_handle)
{
  ...
  {
    RCLCPP_WARN(get_logger(), "Recieved an empty path, this was probaly unintended, ignoring the request.");
    if (rclcpp::ok())
    {
  ...
  while (rclcpp::ok() && !is_goal_distance_tolerance_satisfied)
  {
    ...
    RCLCPP_INFO_THROTTLE(get_logger(), clock, 1000, "Remaining Distance to goal %.4f ...",
                         vox_nav_utilities::getEuclidianDistBetweenPoses(curr_robot_pose, global_path_->poses.back()));

    // check if we have arrived to goal, note the goal is last pose of path
    if (vox_nav_utilities::getEuclidianDistBetweenPoses(curr_robot_pose, global_path_->poses.back()) <
        goal_tolerance_distance_)
    {
      ...
      RCLCPP_INFO(this->get_logger(), "Goal has been reached, Now adjusting correct orientation ...");
      ...
      while (rclcpp::ok() && !is_goal_orientation_tolerance_satisfied)
      {
        ...
        computed_velocity_commands = controller_->computeHeadingCorrectionCommands(curr_robot_pose);
        ...
        vox_nav_utilities::getRPYfromMsgQuaternion(global_path_->poses.back().pose.orientation, nan, nan, goal_psi);
        ...
        if (std::abs(curr_robot_psi - goal_psi) < goal_tolerance_orientation_)
        {
          ...
          // reset the velocity
          cmd_vel_publisher_->publish(geometry_msgs::msg::Twist());
          RCLCPP_INFO(this->get_logger(), "Adjusted correct orientation finishing the action with success.");
          break;
        }
      }
      break;
