...
FastGICPClient::FastGICPClient() : Node("fast_gicp_client_rclcpp_node")
{
  live_cloud_subscriber_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      ...
      std::bind(&FastGICPClient::liveCloudCallback, this, std::placeholders::_1));
  ...
  map_cloud_subscriber_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      ...
      std::bind(&FastGICPClient::mapCloudCallback, this, std::placeholders::_1));
...
void FastGICPClient::gpsOdomCallback(const nav_msgs::msg::Odometry::ConstSharedPtr odom)
{
  std::lock_guard<std::mutex> guard(latest_gps_odom_mutex_);
  latest_gps_odom_ = std::make_shared<nav_msgs::msg::Odometry>(*odom);
}

void FastGICPClient::liveCloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr cloud)
{
  if (map_configured_)
  {
    std::lock_guard<std::mutex> guard(latest_gps_odom_mutex_);
    ...
    if (!pcl_ros::transformPointCloud("base_link", *croppped_map_cloud, *croppped_map_cloud, *tf_buffer_))
    {
      RCLCPP_WARN(get_logger(), "Error Encountered at transfrom, doing nothing");
      return;
    }
    ...
    auto result = vox_nav_utilities::transformPose(tf_buffer_, "map", b, a, transform_tolerance);
...
void FastGICPClient::mapCloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr cloud)
{
...
pcl::Registration<pcl::PointXYZ, pcl::PointXYZ>::Ptr FastGICPClient::createRegistration(std::string method,
                                                                                        int num_threads)
{
  if (method == "GICP")
  {
