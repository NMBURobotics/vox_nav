...
BotanbotAckermannDrive::BotanbotAckermannDrive()
: impl_(std::make_unique<BotanbotAckermannDrivePrivate>())
{
}
...
void BotanbotAckermannDrive::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  impl_->model_ = _model;
  ...
  impl_->joints_[BotanbotAckermannDrivePrivate::STEER_WHEEL] =
    _model->GetJoint(steering_wheel_joint);
  if (!impl_->joints_[BotanbotAckermannDrivePrivate::STEER_WHEEL]) {
    RCLCPP_WARN(
  ...
  impl_->joints_[BotanbotAckermannDrivePrivate::FRONT_RIGHT] = _model->GetJoint(front_right_joint);
  if (!impl_->joints_[BotanbotAckermannDrivePrivate::FRONT_RIGHT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->joints_[BotanbotAckermannDrivePrivate::FRONT_LEFT] = _model->GetJoint(front_left_joint);
  if (!impl_->joints_[BotanbotAckermannDrivePrivate::FRONT_LEFT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->joints_[BotanbotAckermannDrivePrivate::REAR_RIGHT] = _model->GetJoint(rear_right_joint);
  if (!impl_->joints_[BotanbotAckermannDrivePrivate::REAR_RIGHT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->joints_[BotanbotAckermannDrivePrivate::REAR_LEFT] = _model->GetJoint(rear_left_joint);
  if (!impl_->joints_[BotanbotAckermannDrivePrivate::REAR_LEFT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->joints_[BotanbotAckermannDrivePrivate::STEER_RIGHT] =
    _model->GetJoint(right_steering_joint);
  if (!impl_->joints_[BotanbotAckermannDrivePrivate::STEER_RIGHT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->joints_[BotanbotAckermannDrivePrivate::STEER_LEFT] =
    _model->GetJoint(left_steering_joint);
  if (!impl_->joints_[BotanbotAckermannDrivePrivate::STEER_LEFT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->wheel_radius_ = impl_->CollisionRadius(
    impl_->joints_[BotanbotAckermannDrivePrivate::REAR_RIGHT]->GetChild()->GetCollision(id));
  ...
  auto front_right_center_pos = impl_->joints_[BotanbotAckermannDrivePrivate::FRONT_RIGHT]->
    GetChild()->GetCollision(id)->WorldPose().Pos();
  auto front_left_center_pos = impl_->joints_[BotanbotAckermannDrivePrivate::FRONT_LEFT]->
    GetChild()->GetCollision(id)->WorldPose().Pos();
  auto rear_right_center_pos = impl_->joints_[BotanbotAckermannDrivePrivate::REAR_RIGHT]->
    GetChild()->GetCollision(id)->WorldPose().Pos();
  auto rear_left_center_pos = impl_->joints_[BotanbotAckermannDrivePrivate::REAR_LEFT]->
    GetChild()->GetCollision(id)->WorldPose().Pos();

  auto distance = front_left_center_pos - front_right_center_pos;
  impl_->wheel_separation_ = distance.Length();
  RCLCPP_INFO(
    impl_->ros_node_->get_logger(), "Wheel Seperation is: %.2f meters", impl_->wheel_separation_);
  ...
  RCLCPP_INFO(
    impl_->ros_node_->get_logger(), "Wheel Base is: %.2f meters", impl_->wheel_base_);
  ...
  impl_->cmd_vel_sub_ = impl_->ros_node_->create_subscription<geometry_msgs::msg::Twist>(
    ...
    std::bind(&BotanbotAckermannDrivePrivate::OnCmdVel, impl_.get(), std::placeholders::_1));
  ...
  impl_->update_connection_ = gazebo::event::Events::ConnectWorldUpdateBegin(
    std::bind(&BotanbotAckermannDrivePrivate::OnUpdate, impl_.get(), std::placeholders::_1));
}

void BotanbotAckermannDrive::Reset()
{
...
void BotanbotAckermannDrivePrivate::OnUpdate(const gazebo::common::UpdateInfo & _info)
{
  #ifdef IGN_PROFILER_ENABLE
  IGN_PROFILE("BotanbotAckermannDrivePrivate::OnUpdate");
  #endif
...
void BotanbotAckermannDrivePrivate::OnCmdVel(const geometry_msgs::msg::Twist::SharedPtr _msg)
{
...
double BotanbotAckermannDrivePrivate::CollisionRadius(const gazebo::physics::CollisionPtr & _coll)
{
...
void BotanbotAckermannDrivePrivate::UpdateOdometryWorld()
{
...
void BotanbotAckermannDrivePrivate::PublishOdometryTf(const gazebo::common::Time & _current_time)
{
...
void BotanbotAckermannDrivePrivate::PublishWheelsTf(const gazebo::common::Time & _current_time)
{
...
void BotanbotAckermannDrivePrivate::PublishOdometryMsg(const gazebo::common::Time & _current_time)
{
  // Set covariance
...
GZ_REGISTER_MODEL_PLUGIN(BotanbotAckermannDrive)
}  // namespace gazebo_plugins
