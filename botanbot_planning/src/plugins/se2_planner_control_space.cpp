  ...
  const std::string & plugin_name)
{
  ...
  if (!is_enabled_) {
    RCLCPP_INFO(
      node_->get_logger(), "SE2PlannerControlSpace plugin is disabled.");
  } else {
    RCLCPP_INFO(
      node_->get_logger(), "Initializing SE2PlannerControlSpace plugin, selected planner is; %s",
      planner_name_.c_str());
  }
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  if (!getSelectedPlanner(planner_name_, state_space_information_, planner)) {
    RCLCPP_WARN(
      ...
      planner_name_.c_str());
    planner = ompl::base::PlannerPtr(new ompl::control::RRT(control_space_information));
  }
