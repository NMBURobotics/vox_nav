...
def plot_car(x,
             ...
             steer=0.0,
             cabcolor="-r",
             truckcolor="-k"):  # pragma: no cover

    outline = np.array([[
        -BACKTOWHEEL, (LENGTH - BACKTOWHEEL), (LENGTH - BACKTOWHEEL),
...
def linear_mpc_control(xref, xbar, x0, dref):
    ...
    X, U = acado.mpc(0, 1, _x0, X, U, Y, yN, np.transpose(np.tile(Q, T)), Qf,
                     0)
...
def do_simulation(cx, cy, cyaw, ck, sp, dl, initial_state):
    ...
    while MAX_TIME >= time:
        ...
        if show_animation and (isgoal
                               or time > nextPlotTime):  # pragma: no cover
